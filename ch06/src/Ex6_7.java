...
class Ex6_7 {
	...
	// 참조형 반환타입 이해하기.
		...
		Data3 d2 = copy(d); // return tmp => copy(d)의 참조매게변수 => Data3 d2 참조변수에 저장
		
		// 호출스텍에 의해 Data3 d2의 copy는 종료되어 사라지고 main 메서드로 이동하게 되어 출력 d = 0x100 , d2 = 0x200 각각 주소로 반환 실행
		
		
		System.out.println("d.x = " + d.x); // Date3 d 객체의 값을 호출
		System.out.println("d2.x = " + d2.x); // Date3 d2 객체의 값을 호출 (Date d를 참조한)
